...
public class Task
{
    public static void main(String[] args) throws IOException
    {
        ...
        Map<Integer, Deque<Integer>> pos = new HashMap<>(n);
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            {
                int indx = pos.get(x).removeFirst();
                st.update(indx, 0);
            }
            //out.println(num);
            //st.print(out);
    ...
    public static class SegmentsTree
    {
        ...
        public int calc(int n1, int n2)
        {
            ...
            if (n1 == 1 || n2 == 1)
                return 1;
            //int[] key = new int[] { n1, n2 };
            //if (mem.containsKey(key))
            //    return mem.get(key);
            int nod = binaryNOD(n1, n2);
            //mem.put(key, nod);
            return nod;
        }
        
        public int binaryNOD(int n1, int n2)
        {
            if (n1 == 0)
                return n2;
            if (n2 == 0)
            ...
            if ((n1 & 1) == 0)
                return ((n2 & 1) == 0)
                    ? binaryNOD(n1 >> 1, n2 >> 1) << 1
                    : binaryNOD(n1 >> 1, n2);
            else
                return ((n2 & 1) == 0)
                    ? binaryNOD(n1, n2 >> 1)
                    : binaryNOD(n2, n1 > n2 ? n1 - n2 : n2 - n1);
        }
